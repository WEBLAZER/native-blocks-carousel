...
class ThemeStyles
{
    ...
    public function injectButtonColors(): void
    {
        ...
        // IMPORTANT: Only filter if colors don't come from theme or user styles
        // If theme or user has explicitly set colors, we should respect them
        // even if they match core defaults (they might want to use core defaults intentionally)
        $colors_explicitly_defined = !empty($user_settings['styles']['elements']['button'] ?? []) 
                                  || !empty($theme_settings['styles']['elements']['button'] ?? []);

        if (!$colors_explicitly_defined) {
            // Only filter core defaults if colors don't come from theme or user styles
            // This prevents using core defaults that might be stored in core data
